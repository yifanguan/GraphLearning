...
class StandardGNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_fc_layers,
        ...
        # optional simple residual scaling (same formula)
        # self.scale = (1.0 / math.sqrt(num_fc_layers)) if num_fc_layers > 0 else 1.0

        # # (optional) manual reset of parameters (standard Xavier init)
        # self.reset_parameters()

    # def reset_parameters(self):
    #     """Use standard PyTorch initializations (like nn.Linear.reset_parameters())."""
    ...
    def forward(self, x, edge_index):
        ...
        for lin in self.fcs:
            x_in = x
            x = lin(self.act(x))
            # x = x_in + self.scale * x  # residual
            x = x_in + x  # residual
        return self.readout(x)

# ============================================================
# 2. Standard optimizer (global LR, no per-param scaling)
# ============================================================

def make_standard_optimizer(model, base_lr, opt="adam", weight_decay=0.0, momentum=0.9, betas=(0.9, 0.999)):
    params = model.parameters()
    if opt == "sgd":
        return torch.optim.SGD(params, lr=base_lr, momentum=momentum, weight_decay=weight_decay)
    else:
        return torch.optim.Adam(params, lr=base_lr, betas=betas, weight_decay=weight_decay)
...
lrs    = np.linspace(-14, -6, 15)   # add/remove as you like
...
rows = []
folder_name = 'standard_arxiv_small_range'
...
for width in widths:
    for depth in depths:
        for log2lr in lrs:
            ...
            optimizer = make_standard_optimizer(model, base_lr=lr, opt="adam", weight_decay=0.0)
