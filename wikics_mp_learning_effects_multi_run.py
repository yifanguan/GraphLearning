...
from models.dln import DecoupleModel, iGNN, iGNN_V2, iGNN_energy_version, iGNN_energy_version_fc, iGNN_SGConv
...
def run(dataset_name, num_mp_layers, mp_hidden_dim, num_fl_layers, fl_hidden_dim, num_mp_smoothing_layers,
        ...
        folder_name_suffix="", energy_lambda=1.0, energy_threshold=10000, run_index=0):
    # fix_seed()
    ...
    train_idx = dataset.splits_list[run_index]['train']
    valid_idx = dataset.splits_list[run_index]['valid']
    test_idx = dataset.splits_list[run_index]['test']
    ...
    params = {
        ...
        # 'num_fl_layers': num_fl_layers,
        'feature learning weight dim': fl_hidden_dim,
    ...
    try:
        ...
        folder = Path(f"result_loss_{dataset_name}")
...
def main_experiment(dataset_name, num_mp_layers, mp_hidden_dim=3000, num_fl_layers=3, fl_hidden_dim=512, num_mp_smoothing_layers=1,
                    ...
                    freeze=False, skip_connection=False, dropout=0, folder_name_suffix="", energy_lambda=1.0, energy_threshold=10000, run_index=0):
    ...
    train_learning_loss_list, valid_learning_loss_list, test_learning_loss_list = \
            run(dataset_name, num_mp_layers, mp_hidden_dim, num_fl_layers, fl_hidden_dim, num_mp_smoothing_layers,
                ...
                folder_name_suffix=folder_name_suffix, energy_lambda=energy_lambda, energy_threshold=energy_threshold, run_index=run_index)
...
for j, num_mp_layers in enumerate(mp_layers):
    for i in range(num_runs):
        ...
        main_experiment(dataset_name=dataset_name,
                        ...
                        freeze=freeze, skip_connection=skip_connection, folder_name_suffix="", energy_lambda=0, energy_threshold=-1, run_index=i) #0.00001
...
params = {
    'dataset_name': dataset_name,
    'description': 'SGConv followed by one activation and one output layer',
    # 'num_mp_layers': num_mp_layers,
    'feature learning weight dim': mp_hidden_dim,
    # 'num_fl_layers': num_fl_layers,
    'optimizer_lr': optimizer_lr,
    'act': 'F.gelu',
    # 'freeze': freeze,
