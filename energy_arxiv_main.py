...
def train(model, data, train_idx, optimizer, criterion, energy_lambda, energy_threshold):
    ...
    loss.backward()
    optimizer.step()
    return loss.item(), energy_loss.item(), train_learning_loss.item(), norms_per_layer, norm_energy_loss
...
def run(dataset_name, num_mp_layers, mp_hidden_dim, num_fl_layers, fl_hidden_dim,
    ...
    try:
        ...
        folder = Path(f"result_energy_loss_simple")
...
def main_experiment(dataset_name, num_mp_layers, mp_hidden_dim=3000, num_fl_layers=3, fl_hidden_dim=512, optimizer_lr=0.01,
    ...
    test_learning_loss_list, norms_list, norm_energy_loss_list = \
            run(dataset_name, num_mp_layers, mp_hidden_dim, num_fl_layers, fl_hidden_dim,
...
# Create folder for results
folder = Path(f"result_energy_loss_multi_run_simple")
folder.mkdir(parents=True, exist_ok=True)
folder_name = folder.name
...
for energy_lambda in lambdas:
    ...
    main_experiment(dataset_name=dataset_name,
                    ...
                    freeze=freeze, skip_connection=skip_connection, folder_name_suffix="", energy_lambda=energy_lambda, energy_threshold=-1) #0.00001
...
num_layers = num_mp_layers + num_fl_layers
...
ax.set_yscale("log")
plt.legend()
plt.savefig('{}/{}_normalized_energy_loss_with_different_lambda_{}.png'.format(folder_name, dataset_name, get_timestamp()))
plt.clf()  # Clear the current figure for the next plot
plt.close(fig)
...
ax.set_yscale("log")
plt.legend()
plt.savefig('{}/{}_train_loss_with_different_lambda_{}.png'.format(folder_name, dataset_name, get_timestamp()))
plt.clf()  # Clear the current figure for the next plot
plt.close(fig)
...
ax.set_yscale("log")
plt.legend()
plt.savefig('{}/{}_valid_loss_with_different_lambda_{}.png'.format(folder_name, dataset_name, get_timestamp()))
plt.clf()  # Clear the current figure for the next plot
plt.close(fig)
...
ax.set_yscale("log")
plt.legend()
plt.savefig('{}/{}_test_loss_with_different_lambda_{}.png'.format(folder_name, dataset_name, get_timestamp()))
plt.clf()  # Clear the current figure for the next plot
plt.close(fig)
...
plt.legend()
plt.savefig('{}/{}_test_accuracy_with_different_lambda_{}.png'.format(folder_name, dataset_name, get_timestamp()))
plt.clf()  # Clear the current figure for the next plot
plt.close(fig)
...
plt.legend()
plt.savefig('{}/{}_train_accuracy_with_different_lambda_{}.png'.format(folder_name, dataset_name, get_timestamp()))
plt.clf()  # Clear the current figure for the next plot
plt.close(fig)
...
plt.legend()
plt.savefig('{}/{}_valid_accuracy_with_different_lambda_{}.png'.format(folder_name, dataset_name, get_timestamp()))
plt.clf()  # Clear the current figure for the next plot
plt.close(fig)
...
fig, ax = add_hyperparameter_text(params)
# Plot with evenly spaced points
ax.plot(range(len(lambdas)), best_vals, label=f'Best valid accuracy', linestyle='-', marker='o')
ax.plot(range(len(lambdas)), best_tests, label=f'Best test accuracy', linestyle='-', marker='o')
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('{}/{}_best_accuracy_with_different_lambda_{}.png'.format(folder_name, dataset_name, get_timestamp()))
plt.clf()  # Clear the current figure for the next plot
plt.close(fig)
