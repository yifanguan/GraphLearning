...
from models.dln import DecoupleModel, iGNN, iGNN_V2, iGNN_energy_version, iGNN_energy_version_fc, iGNN_mp_fl_mp_version
...
def train(model, data, train_idx, optimizer, criterion, energy_lambda, energy_threshold):
    ...
    energy_loss = dirichlet_energy(embedding.detach(), data.edge_index)
...
def run(dataset_name, num_mp_layers, mp_hidden_dim, num_fl_layers, fl_hidden_dim, num_mp_smoothing_layers,
    ...
    try:
        ...
        folder = Path(f"result_loss_simple_mp_after_fl")
...
folder = Path(f"result_simple_mp_after_fl")
folder.mkdir(parents=True, exist_ok=True)
folder_name = folder.name
