...
def objective(trial):
    # 超参数搜索空间
    num_mp_layers = trial.suggest_int("num_mp_layers", 1, 4)
    num_fl_layers = trial.suggest_int("num_fl_layers", 1, 7)
    fl_hidden_dim = trial.suggest_categorical("fl_hidden_dim", [64, 128, 256, 512, 1024])
    ...
    best_val, best_test, *_ = main_experiment(
        ...
        fl_hidden_dim=fl_hidden_dim,
        optimizer_lr=optimizer_lr,
        total_epoch=3000,   # 为了快速调参可以减少 epoch
    ...
    return best_val  # 以 validation accuracy 为优化目标
